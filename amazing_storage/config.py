...
def mask_sensitive_value(value: str) -> str:
    """Mask sensitive values for logging (show first 4 chars, then asterisks)."""
...
class AppConfig:
    ...
    def load(cls, path: str = CONFIG_FILE) -> 'AppConfig':
        ...
        for i, bucket_data in enumerate(config_data.get("buckets", [])):
            # Get credential filename from environment or fallback to config file value
            ...
            # Build BucketConfig explicitly to handle optional fields
        ...
        # Log Dropbox OAuth settings for debugging but mask sensitive values
